...
variable "role_bindings" {
  type = list(object({
    project = string       # (Required) The project ID where the roles will be applied.
    roles   = list(string) # (Required) List of IAM roles to bind to the member (e.g., roles/editor, roles/viewer).
  }))
}
